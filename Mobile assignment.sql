...
DIM_LOCATION s
JOIN
fact_transactions f ON f.IDlocation = s.IDlocation
JOIN
...
/*2.) What state in the US is buying more "SAMSUNG" Cellphones?*/
 ...
 Select top 1
 s.state, n.Manufacturer_name, s.Country, count(f.IDModel) as number_of_transactions
 from
 DIM_location s
 Left JOIN
 fact_transactions f ON f.IDlocation = s.IDlocation
 Left JOIN
 DIM_Model m ON m.IDmodel = f.IDmodel
 Left JOIN
 DIM_manufacturer n ON n.IDManufacturer = m.IDManufacturer
 where
Country = 'US' AND Manufacturer_name = 'SAMSUNG'
Group by n.Manufacturer_name, s.Country,  s.state
order by count(f.IDModel) DESC
...
select
m.IDmodel, l.state, l.Zipcode, count(Totalprice) AS no_of_transactions

from
  DIM_Model m                                                      

Left JOIN
fact_transactions f ON f.IDModel = m.IDmodel
Left JOIN
DIM_location l ON l.IDLocation = f.IDlocation
...
Select Top 1
Min(Totalprice) AS Price, n.manufacturer_name, m.Model_Name
from
DIM_manufacturer n
Left Join
DIM_model m ON n.IDManufacturer = m.IDmanufacturer                  
Left Join
fact_transactions f ON f.IDmodel = m.IDMODEL
Group by manufacturer_name, m.Model_Name
Order by Price asc
...
SELECT TOP 5 
 dm.IDModel, dm.Model_Name, sum(ft.TotalPrice) Sales_sum, sum(ft.Quantity) Quantity_sum, FORMAT(sum(ft.TotalPrice)/count(ft.Quantity), 'N2') Average_price 
FROM DIM_MODEL dm 
...
WHERE dm.IDManufacturer IN (
	...
	JOIN DIM_MODEL DM1 ON DM.IDManufacturer = DM1.IDManufacturer
	JOIN FACT_TRANSACTIONS ft ON ft.IDModel = DM1.IDModel
	GROUP BY DM.IDManufacturer
	ORDER BY SUM(ft.Quantity) DESC
	
)
GROUP BY dm.IDModel, dm.Model_Name
ORDER BY sum(ft.TotalPrice)/count(ft.Quantity) DESC 
...
select
Customer_Name, Year(Date) as S_year, avg(totalprice) as AvgAmount
from
DIM_CUSTOMER C
Left Join FACT_TRANSACTIONS f ON C.IDCustomer = f.IDCustomer
...
select IDModel, Model_Name from(
select TOP 5
m.IDModel, Model_name, Year(Date) as S_year, Sum(f.Quantity) as Tot_quantity
from
FACT_TRANSACTIONS f
Left Join DIM_MODEL m ON f.IDModel = m.IDModel
where Year(Date) = 2008
Group by m.IDModel, Model_Name, Year(Date)
Order by Sum(f.Quantity) DESC) AS T1
Intersect
select IDModel, Model_Name from(
select TOP 5
m.IDModel, Model_name, Year(Date) as S_year, Sum(f.Quantity) as Tot_quantity
...
where Year(Date) = 2009
Group by m.IDModel, Model_Name, Year(Date)
Order by Sum(f.Quantity) DESC) AS T2


/* 8.) Show the manufacturers with the second top sales in the year of 2009 and the manufacturers with the 2nd top sales in the year of 2010.*/

select * from(
select *, row_number() over(Partition by p_year order by Tot_Sales DESC) as RNUM  from(
select
Manufacturer_Name, year(f.date) as p_year, SUM(TotalPrice) as Tot_Sales
from
DIM_MANUFACTURER m1
Left Join DIM_MODEL m2 ON m1.IDManufacturer = m2.IDManufacturer
Right Join FACT_TRANSACTIONS f on m2.IDModel = f.IDModel
where year(f.date) IN (2009, 2010)
group by Manufacturer_Name, year(f.date)) AS T1) AS T2
where 
RNUM = 2


/* 9.) Show the manufacturers that sold the cellphones in 2010 but didn't in 2009.*/
...
right join DIM_MODEL AS m ON  f.IDModel= m.IDModel
RIGHT JOIN DIM_MANUFACTURER AS m1 ON m.IDManufacturer= m1.IDManufacturer
RIGHT JOIN DIM_DATE AS d ON d.DATE=f.DATE
WHERE
YEAR=2010 
GROUP BY
Manufacturer_Name
...
right join DIM_MODEL AS m ON  f.IDModel=m.IDModel
RIGHT JOIN DIM_MANUFACTURER AS m1 ON m.IDManufacturer=m1.IDManufacturer
RIGHT JOIN DIM_DATE AS d ON f.DATE=d.DATE
WHERE
YEAR=2009
GROUP BY
Manufacturer_Name
...
YEAR,
SUM(TOTALPRICE) AS TOT_SALES,
COUNT(CUSTOMER_NAME) AS Customer_count,
TotalPrice,
Quantity
from
FACT_TRANSACTIONS as f
right join DIM_CUSTOMER as c ON f.IDCustomer= c.IDCustomer
right join DIM_DATE AS d ON d.DATE= f.DATE
group by
